...
function show_image_uploader () {
	...
	file_upload_limit : "<?php if($image_limit){echo $image_limit;}else{echo '0';} ?>",
...
/* ]]> */
	...
	<?php
	} else { if($image_limit=='0'){echo NO_IMAGES;}else{
	?>
	...
	if($_SESSION["file_info"])
	{
		foreach($_SESSION["file_info"] as $image_id=>$val)
		{
			?>
                  ...
                  <div class="imageBox" id="div_<?php echo $image_id;?>">
            <div class="imageBox_theImage" id="photo_<?php echo $image_id;?>" ><img src="<?php echo site_url().'/'.$upload_folder_path.'tmp/'.$image_id.'.jpg'; ?>" height="80" ></div>
            <div class="imageBox_label"><span><a href="javascript:void(0);" id="a_photo_<?php echo $image_id;?>" onClick="javascript:removePhoto('','<?php echo $image_id;?>');"><img src="<?php echo bloginfo('template_url'); ?>/images/x.png" class="img_delete" alt="" ></a></span></div>
		</div>
			<?php
		}
	}
	...
	<span class="message_note"><?php if($_REQUEST['pid']){ _e(IMAGE_ORDERING_MSG);}else{ _e(IMAGE_SAVE_ORDERING_MSG);}?></span>
<?php
if($_REQUEST['pid'])
{
?>	
<div id="insertionMarker">
	<img src="<?php echo bloginfo('template_url'); ?>/images/marker_top.gif">
	<img src="<?php echo bloginfo('template_url'); ?>/images/marker_middle.gif" id="insertionMarkerLine">
	<img src="<?php echo bloginfo('template_url'); ?>/images/marker_bottom.gif">
</div>
...
// fix for deleting images and not being able to upload again.
//alert('start');
//alert(tmp_image_id);
if(tmp_image_id!='undefined'){
var uploads_remaining = swfu.getStats().files_queued; // do calculation.
var stats = swfu.getStats();
//alert(stats.successful_uploads);
stats.successful_uploads--;
swfu.setStats(stats);
var upload_count = swfu.getStats().successful_uploads;
swfu.setFileQueueLimit(swfu.settings.file_upload_limit - (upload_count + uploads_remaining));
//this.uploadError(file, 300, serverData);
//alert(stats.successful_uploads);
}
// end fix for deleting images and not being able to upload again.	
