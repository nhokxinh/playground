...
class bcn_breadcrumb
{
	...
	public function bcn_breadcrumb($title = '', $prefix = '', $suffix = '', $anchor = NULL, $linked = false)
	{
		...
		$this->title = __($title);
		//Set the prefix
		$this->prefix = __($prefix);
		//Set the suffix
		$this->suffix = __($suffix);
	...
	public function set_anchor($template, $url)
	{
		...
		if($template == '')
		{
			$template = __('<a title="Go to %title%" href="%link%">');
		}
		//Set the anchor, we strip tangs from the title to prevent html validation problems
		$this->anchor = str_replace('%title%', strip_tags($this->title), str_replace('%link%', $url, __($template)));
...
class bcn_breadcrumb_trail
{
	...
	function bcn_breadcrumb_trail()
	{
		...
		(
			...
			'home_anchor' => __('<a title="Go to %title%" href="%link%">'),
			...
			'blog_anchor' => __('<a title="Go to %title%" href="%link%">'),
			...
			'current_item_anchor' => __('<a title="Reload the current page." href="%link%">'),
			...
			'post_page_anchor' => __('<a title="Go to %title%" href="%link%">'),
			...
			'post_post_anchor' => __('<a title="Go to %title%" href="%link%">'),
			...
			'search_prefix' => __('Search results for &#39;'),
			...
			'search_anchor' => __('<a title="Go to the first page of search results for %title%." href="%link%">'),
			//Tag related stuff
			...
			'author_anchor' => __('<a title="Go to the first page of posts by %title%." href="%link%">'),
			...
			'category_anchor' => __('<a title="Go to the %title% category archives." href="%link%">'),
			...
			'archive_category_prefix' => __('Archive by category &#39;'),
			...
			'archive_post_tag_prefix' => __('Archive by tag &#39;'),
			//Suffix for tag archives, place inside of the current_item suffix
			'archive_post_tag_suffix' => '&#39;',
			'date_anchor' => __('<a title="Go to the %title% archives." href="%link%">'),
	...
	{ 
		...
		if(is_array($bcn_object))
		{
			...
			foreach($bcn_object as $term)
			{
				...
				$bcn_breadcrumb->set_title($bcn_breadcrumb->get_title() . $this->opt[$taxonomy . '_prefix'] . str_replace('%title%', $term->name, str_replace('%link%', get_term_link($term, $taxonomy), $this->opt[$taxonomy . '_anchor'])) .	$term->name . '</a>' . $this->opt[$taxonomy . '_suffix']);
				$is_first = false;
			}
		}
		else
		{
			...
			$bcn_breadcrumb->set_title(__('Un' . $taxonomy->name));
		}
	}
	/**
	 * A Breadcrumb Trail Filling Function
	...
	function do_home()
	{
		...
		{
			if(!get_option( 'ptthemes_breadcrumbs_hide' )){
			//Place the breadcrumb in the trail, uses the constructor to set the title, prefix, and suffix, get a pointer to it in return
			
			//Deal with the anchor
			if($type->post_type=='place'){
				if(!get_option( 'ptthemes_breadcrumbs_hide' )){$breadcrumb = $this->add(new bcn_breadcrumb($this->post_type_archive_title(get_post_type_object($type->post_type)), $this->opt['post_page_prefix'],
				$this->opt['post_page_suffix']));
				$breadcrumb->set_anchor($this->opt['post_place_anchor'], get_post_type_archive_link($type->post_type));}}
			elseif($type->post_type=='event'){
				if(!get_option( 'ptthemes_breadcrumbs_hide' )){
				...
				$this->opt['post_page_suffix']));
				$breadcrumb->set_anchor($this->opt['post_event_anchor'], get_post_type_archive_link($type->post_type));}}
			else{$breadcrumb = $this->add(new bcn_breadcrumb($this->post_type_archive_title(get_post_type_object($type->post_type)), $this->opt['post_page_prefix'],
				$this->opt['post_page_suffix']));
		...
		else if(isset($type->taxonomy) && !$this->is_builtin($wp_taxonomies[$type->taxonomy]->object_type[0]))
		{
			if(!get_option( 'ptthemes_breadcrumbs_hide' )){
			//Place the breadcrumb in the trail, uses the constructor to set the title, prefix, and suffix, get a pointer to it in return
			$breadcrumb = $this->add(new bcn_breadcrumb($this->post_type_archive_title(get_post_type_object($wp_taxonomies[$type->taxonomy]->object_type[0])), $this->opt['post_page_prefix'],$this->opt['post_page_suffix']));
			//Deal with the anchor
