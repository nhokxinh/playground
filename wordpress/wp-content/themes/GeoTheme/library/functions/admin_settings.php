				...
				if($value['type'] == 'file')
				{
					if($_FILES)
					{
						...
						if($_FILES[$image_var]['name'] && $_FILES[$image_var]['size']>0)
						{
							...
							{
								$imagepath1 = site_url() ."/wp-content/uploads/".$name;
...
<form method="post" enctype="multipart/form-data">
  ...
  <input class="text_input" name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="text" value="<?php if ( get_settings( $value['id'] ) != "") { echo get_settings( $value['id'] ); } else { echo $value['std']; } ?>" />
   <p style="height:100%; overflow: hidden;"><input type="file" name="<?php echo $value['id']; ?>_image" id="<?php echo $value['id']; ?>_image" style="width:245px; float:left;" /><input type="submit" value="<?php _e('Upload');?>" name="Upload" style="width:100px; float:left;"  /> </p>
  ...
  <?php
		...
		option_wrapper_header($value);
		?>
  <input class="text_input" name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="<?php echo $value['type']; ?>" value="<?php if ( get_settings( $value['id'] ) != "") { echo stripslashes(get_settings( $value['id']) ); } else { echo $value['std']; } ?>" />
  <?php
		option_wrapper_footer($value);
		break;
		case 'multiselect':
