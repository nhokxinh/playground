...
class Tax_Meta_Class {
	...
	public function insert_images() {
		...
		foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
			...
			$li 	.= "<a title='" . __( 'Remove this image' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$term_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . __( 'Remove' ) . "' /></a>";
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
		...
		$this->show_field_begin( $field, $meta );
			...
			if ( ! empty( $meta ) ) {
				...
				foreach ( $meta as $att ) {
					...
					echo "<li>" . wp_get_attachment_link( $att, '' , false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}|{$post->ID}|{$field['id']}|{$att}'>" . __( 'Remove' ) . "</a>)</li>";
				}
				echo '</ol>';
			}
	...
	public function show_field_image( $field, $meta ) {
		...
		if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
			...
			$html .= "<input class='at-delete_image_button' type='button' rel='".$field['id']."' value='Remove Image' />";
		}else{
