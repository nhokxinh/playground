...
define ('GT_VERSION', '1.0.0');																		// GeoTheme version of this script 
...
##########################
		...
		$url_parts = explode("/files/", $this->src);
		
		if($url_parts[1]){
		$bid_src = '/files/'.$url_parts[1];
		$src_src = str_replace("/files/", "/wp-content/blogs.dir/$blog_id/files/", $bid_src);
		}else{
		$url_parts2 = explode("/wp-content/", $this->src);
		if($url_parts2[1]){
		$bid_src = '/wp-content/'.$url_parts2[1];
		$src_src = str_replace("/files/", "/wp-content/blogs.dir/$blog_id/files/", $bid_src);
		}else{$src_src = $this->src;}
		}
		//echo $src_src;
		$src_chk = $_SERVER['DOCUMENT_ROOT'] . '/' . $src_src;
		 if(@file_exists($src_chk)) {
             $this->src = $src_src;
        }
		}
#########################
