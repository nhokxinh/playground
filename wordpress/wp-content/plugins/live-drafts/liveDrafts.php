<?php
/*
Plugin Name: Live Drafts
Description: Adds ability to create a draft of a live page or post.
Author: Stephen Sandison
Author URL: http://www.designbuildhost.co.uk/
Version: 3.0.2
*/
...
if (!class_exists('liveDrafts')) {
	...
	class liveDrafts {
		...
		function adminHead () {
			...
			if (in_array($post->post_type, array('post', 'page', 'ait-dir-item', 'ait-dir-event', 'ait-dir-review')) && $post->post_status == 'publish') {
				?>
		...
		function prePostUpdate ($id) {
			...
			if (!in_array($_POST['post_type'], array('post', 'page', 'ait-dir-item', 'ait-dir-event', 'ait-dir-review'))) {
				return $id;
			}
