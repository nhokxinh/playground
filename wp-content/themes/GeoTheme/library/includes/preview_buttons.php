...
?>
<a href="<?php echo site_url();?>/?ptype=post_listing&backandedit=1<?php if($_REQUEST['pid']){ echo '&pid='.$_REQUEST['pid'];} if($_SESSION['property_info']['price_select']){echo '&pkg='.$_SESSION['property_info']['price_select'];}?><?php if($_REQUEST['renew']){echo '&renew=1';}?><?php if($_REQUEST['upgrade']){echo '&upgrade=1';}?>" class="b_goback fl" ><?php _e(PRO_BACK_AND_EDIT_TEXT);?></a>
