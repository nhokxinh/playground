<?php
global $wpdb,$multiregion_db_table_name,$multicity_db_table_name ;
add_column_if_not_exist($multiregion_db_table_name, 'cat_ex');
add_column_if_not_exist($multiregion_db_table_name, 'home_desc', 'TEXT NOT NULL');
add_column_if_not_exist($multiregion_db_table_name, 'meta_desc', 'TEXT NOT NULL');



if($_POST['cityact'] == 'addregion')
{
	$id  = $_POST['id'];
	...
	if($id!='')
	{
		$wpdb->query("update $multiregion_db_table_name set regionname=\"$regionname\",region_slug=\"$region_slug\", lat=\"$lat\", lng=\"$lng\", scall_factor=\"$scall_factor\",categories=\"$categories\",cat_ex=\"$cat_ex\",is_zoom_home=\"$is_zoom_home\",is_default=\"$default\",sortorder=\"$sortorder\",cities=\"$cities\",home_desc=\"$home_desc\",meta_desc=\"$meta_desc\" where region_id=\"$id\"");
	}else
	{
		$wpdb->query("insert into $multiregion_db_table_name (regionname,region_slug,lat,lng,scall_factor,categories,cat_ex,is_zoom_home,is_default,sortorder,cities,home_desc,meta_desc) values (\"$regionname\",\"$region_slug\",\"$lat\",\"$lng\",\"$scall_factor\",\"$categories\",\"$cat_ex\",\"$is_zoom_home\",\"$default\",\"$sortorder\",\"$cities\",\"$home_desc\",\"$meta_desc\")");
	}
	$location = site_url()."/wp-admin/admin.php";
	echo '<form action="'.$location.'" method=get name="city_success">
	<input type=hidden name="page" value="region"><input type=hidden name="msg" value="success"></form>';
...
{
	$pid = $_REQUEST['id'];
	$citysql = "select * from $multiregion_db_table_name where region_id=\"$pid\"";
	$cityinfo = $wpdb->get_results($citysql);
}
?>

<form action="<?php echo site_url()?>/wp-admin/admin.php?page=region&pagetype=addedit&pid=<?php echo $_REQUEST['id'];?>" method="post" name="price_frm">
  <input type="hidden" name="cityact" value="addregion">
...
?>
  ...
  <table width="75%" cellpadding="3" cellspacing="3" class="widefat post fixed" >
    <tr>
      <td width="14%"><?php _e('Region Name');?></td>
      <td width="86%">:
        <input type="text" name="regionname" id="regionname" value="<?php echo $cityinfo[0]->regionname;?>"><input type="button" class="b_submit" value="Set Address on Map" onclick="codeAddress()" style="float:none;"/></td>
    </tr>
     <tr>
      <td width="14%"><?php _e('Region Slug');?></td>
      <td width="86%">:
        <input type="text" name="region_slug" id="region_slug" value="<?php echo $cityinfo[0]->region_slug;?>"></td>
    </tr>
     <tr>
      <td><?php _e('Map');?></td>
...
function codeAddress() {
    ...
    var address = document.getElementById("regionname").value;
...
<div id="map_canvas" style=" height:300px; width:435px;"  class="form_row clearfix"></div>
    ...
    <tr>
     ...
     <tr>
      <td><?php _e('Set as default Region');?></td>
      <td>:
...
<textarea  name='meta_desc' id='meta_desc' cols='60' rows='5'  style='width:620px'><?php echo $cityinfo[0]->meta_desc;?></textarea>
    ...
    <tr>
      ...
      <td><input type="submit" name="submit" value="<?php _e('Submit');?>" onclick="return check_frm();" class="button-secondary action" >
        &nbsp;
        <input type="button" name="cancel" value="<?php _e('Cancel');?>" onClick="window.location.href='<?php echo site_url()?>/wp-admin/admin.php?page=region'" class="button-secondary action" ></td>
    </tr>
  </table>
</form>
<script>
function check_frm()
{
	...
	{
		alert("<?php _e('Please enter Region Name');?>");
		return false;
	}
	if(document.getElementById('region_slug').value=='')
	{
		alert("<?php _e('Please enter Region Slug');?>");
		return false;
	}
	return true;
}
